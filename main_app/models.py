...
class CustomUser(AbstractUser):
    ...
    avatar = models.ImageField(
        ...
        max_length=100,
        upload_to=get_user_avatar_dir_path,
        blank=True,
    ...
    discord_username = models.CharField(
        verbose_name=_('Имя пользователя Discord'), max_length=40, blank=True
    )

    battlenet_username = models.CharField(
        verbose_name=_('Имя пользователя Battle.net'), max_length=40, blank=True
    )
    ...
    twitch_link = models.CharField(
        verbose_name=_('Ссылка на ваш Twitch'), max_length=200, null=True, blank=True
    )

    dress_room_link = models.CharField(
        verbose_name=_('Ссылка на модель персонажа'), max_length=200, null=True, blank=True
    )

    subscribe_newsletter = models.BooleanField(
        default=True, verbose_name=_('Разрешить отправлять новостную рассылку по почте')
    )

    subscribe_notify = models.BooleanField(
        default=True, verbose_name=_('Разрешить отправлять уведомления по почте')
    )
    ...
    game_class = MultiSelectField(
        verbose_name=_('Игровой класс'), choices=GAME_CLASSES, max_choices=3, max_length=5, blank=True
    )
    ...
    def get_user_game_classes_data(self) -> dict[str, dict[str, str]]:
        ...
        for class_id in self.game_class:
            for class_id_and_verbose_name in self.GAME_CLASSES:
                if (
                    int(class_id) == class_id_and_verbose_name[0]
                    and class_id_and_verbose_name[1] not in game_classes_data
                ):
...
class ServiceInfo(models.Model):
    ...
    server_email_login = models.CharField(
        verbose_name='Серверный адрес электронной почты', max_length=100, blank=True
    )
    server_email_password = models.CharField(
        verbose_name='Серверный пароль приложения почты', max_length=100, blank=True
    )
    ...
    discord_token = models.CharField(
        verbose_name='Discord - Токен',
        help_text='Токен заполняется автоматически.',
        max_length=100,
        blank=True,
    )
    ...
    twitch_login = models.CharField(
        ...
        blank=True,
        help_text="""
        Данное поле необязательно для заполнения и носит исключительно информационный характер.""",
    )
    twitch_password = models.CharField(
        verbose_name='Twitch - Пароль',
        max_length=100,
        blank=True,
        help_text="""
        Данное поле необязательно для заполнения и носит исключительно информационный характер.""",
    )
    twitch_client_id = models.CharField(
        verbose_name='Twitch - Идентификатор клиента',
        max_length=100,
        blank=True,
        help_text='Добывается по адресу: dev.twitch.tv/console',
    )
    twitch_client_secret = models.CharField(
        verbose_name='Twitch - Секретный код клиента',
        max_length=100,
        blank=True,
        help_text='Добывается по адресу: dev.twitch.tv/console',
    )
    twitch_token = models.CharField(
        verbose_name='Twitch - Токен',
        help_text='Токен заполняется автоматически.',
        max_length=100,
        blank=True,
    )
    ...
    recaptcha_public_key = models.CharField(
        verbose_name='reCAPTCHA - Ключ Сайта',
        help_text='Добывается по адресу: https://www.google.com/recaptcha/about/',
        max_length=100,
        blank=True,
    )
    recaptcha_private_key = models.CharField(
        verbose_name='reCAPTCHA - Секретный Ключ',
        help_text='Добывается по адресу: https://www.google.com/recaptcha/about/',
        max_length=100,
        blank=True,
    )

    apikey_for_captcha_solution = models.CharField(
        verbose_name='Ключ от сервиса решения капчи',
...
class TwitchStreamerInfo(models.Model):
    ...
    def _rogue(self):
        return format_html('<span style="color: rgb({});">{}</span>', CLASS_COLORS['rogue'], self.rogue)
    ...
    def _mage(self):
        return format_html('<span style="color: rgb({});">{}</span>', CLASS_COLORS['mage'], self.mage)
    ...
    def _druid(self):
        return format_html('<span style="color: rgb({});">{}</span>', CLASS_COLORS['druid'], self.druid)
...
class DressingRoom(models.Model):
    ...
    creator = models.ForeignKey(
        CustomUser, on_delete=models.CASCADE, null=True, verbose_name='Создатель комнаты'
    )
...
class Guides(models.Model):
    ...
    content = models.TextField(
        verbose_name='Текст гайда',
        max_length=150000,
        help_text="""
    <br>Текст гайда не экранируется, поэтому прямо в текстовом поле можно записывать любые HTML теги.
    <br><br>WowHead тултипы будут выводится автоматически при использовании тега &lt;a&gt;<br><br>
    Внутри тега &lt;a&gt; обязательно необходимо поместить хотя бы один символ текста,
    для отображения ссылки.
    <br><br>&lt;a target="_BLANK" href="https://www.wowhead.com/wotlk/ru/spell=52985"&gt;Исповедь&lt;/a&gt;
    """,
    )
    ...
    guide_creator = models.ForeignKey(
        CustomUser, on_delete=models.CASCADE, verbose_name='Создатель гайда', blank=True, null=True
    )
    ...
    class Meta:
        ...
        # поиск по индексу будет использован только
        # когда будет осуществлён поиск с указанием этих двух полей
...
class Category(models.Model):
    ...
    cat_creator = models.ForeignKey(
        CustomUser, on_delete=models.CASCADE, verbose_name='Создатель категории', blank=True, null=True
    )
...
class Comments(MPTTModel):
    ...
    guide = models.ForeignKey(
        Guides, on_delete=models.CASCADE, related_name='comments', verbose_name='Гайд'
    )
    ...
    parent = TreeForeignKey(
        'self', null=True, blank=True, on_delete=models.CASCADE, verbose_name='Родительский комментарий'
    )
