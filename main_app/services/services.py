...
def humanize_size(bytes_size: int, decimals: int=1) -> str:
    ...
    # return f'{round(bytes_size / pow(1024, power), decimals)} {suffixes[power]}'
    # return f'{bytes_size / pow(1024, power):.{decimals}f} {suffixes[power]}'
    return f'{bytes_size / pow(1024, power):.{decimals}f}'.rstrip('0').rstrip('.') + f' {suffixes[power]}'
